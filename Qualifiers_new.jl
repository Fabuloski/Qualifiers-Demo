...
# ╔═╡ 56d1aa40-8d98-11ed-0498-e7f48f341480
using PlutoUI, CSV, DataFrames, CairoMakie, Random, MLJ, ColorSchemes, ScikitLearn

# ╔═╡ c929adc3-cd1d-42cc-b74a-821c225874fa
begin
	@sk_import decomposition: PCA
	@sk_import discriminant_analysis: LinearDiscriminantAnalysis
end

# ╔═╡ c417c280-7413-4a8d-b53a-4d44136972f7
TableOfContents()

# ╔═╡ a6e8f308-0629-41f2-8e93-0a6b9d8ef978
colors = ColorSchemes.tab10

# ╔═╡ 56547ca2-8aae-4265-ac4a-02dd76486421
md"""
## Data Prep
"""

# ╔═╡ 522b1b64-fac3-4398-96fe-0254c708e6fa
begin
	data = CSV.read("C:\\Users\\gbeng\\Downloads\\Occupancy.csv\\Occupancy.csv",
			 		DataFrame)
	select!(data, Not("date"))
end

# ╔═╡ 46bfd57b-ca13-4985-b877-9c895462e63b
X = Matrix(select(data, Not("Occupancy")))

# ╔═╡ 6a76c45f-439f-4ad5-93bd-38c85883f8fd
for i in 1:size(X)[2]
		x̄ = mean(X[:, i])
		σ = std(X[:, i])
		X[:, i] = (X[:, i] .- x̄)/σ
...
# ╔═╡ 5df47caa-db9b-4188-b88f-1b8b086c726b
X

# ╔═╡ 5a12ba00-2ca7-4a09-bb1f-32354e8942af
md"""
## PCA
"""

# ╔═╡ 1217da37-8b38-4069-9083-b6dc0a7b95f1
begin
	pca = PCA(n_components=2)
	transformed_pca = pca.fit_transform(X)
end

# ╔═╡ 552c10f8-6375-4d1a-b86f-50396c767333
variance = sum(pca.explained_variance_ratio_)

# ╔═╡ 47e4069d-3f77-4d95-8b0a-de968fa5d357
subset = partition(1:nrow(data), 0.95, shuffle=true, stratify=data[:, "Occupancy"])[2]

# ╔═╡ 316d821a-781e-4a2c-b8cb-ad801802e532
nrow(data)

# ╔═╡ f89d24fd-6607-4b30-bbc7-a9d5cb5e40a3
length(subset)

# ╔═╡ c5a5a31f-8851-4e6e-b0a4-b110630d6956
begin
	local fig = Figure()
	local ax=Axis(fig[1,1], xlabel="principal component 1", 
				   ylabel="principal component 2")

	scatter!(transformed_pca[subset, 1], transformed_pca[subset, 2],
			 color=colors[data[subset, "Occupancy"] .+ 1])

	local occupied = [MarkerElement(color = colors[2] , marker = :circle, 
					  markersize = 15)]
	local unoccupied = [MarkerElement(color = colors[1] , marker = :circle,
						markersize = 15)]
	
	Legend(fig[1, 2], [occupied, unoccupied], ["occupied", "unoccupied"],
		   patchsize = (25, 25), rowgap = 5)
	
	fig
end

# ╔═╡ 069999f7-7594-45d3-b903-2e49c706044e
md"""
## LDA
"""

# ╔═╡ 6830ef94-db0b-4c54-b710-30c0716203f5
begin
	lda = LinearDiscriminantAnalysis()
	transformed_lda = lda.fit_transform(X, data[:, "Occupancy"])
end

# ╔═╡ c5fe6be9-2d5d-410e-8608-1f52afc4b549
begin
	local fig = Figure()
	local ax=Axis(fig[1,1], xlabel="linear discriminant 1", 
				   ylabel="linear discriminant 2")

	scatter!(transformed_lda[subset, 1], zeros(length(subset)),
			 color=colors[data[subset, "Occupancy"] .+ 1])
...
# ╔═╡ 2eaaed0d-5c56-4d20-ad62-bd112d2ac599
y_pred = [lda.predict(reshape(X[i, :], (1, 5)))[1] for i in 1:nrow(data)]

# ╔═╡ 9b67a616-cf2a-4348-9cd1-3169a8e553a0
sum(y_pred.==data[:, "Occupancy"])/nrow(data)

# ╔═╡ af6f9af7-da92-4238-9d2e-0fb8ad2d3b58
md"""
## Jackknife Classification
"""

# ╔═╡ 96ad75a0-159c-44f6-a8ec-4bd7b823a2dd
begin
	occupied_acc = 0
	unoccupied_acc = 0
	X_subset = X[subset, :]
	for (i, id) in enumerate(subset)
		X_test = reshape(X_subset[i, :], (1, 5))
		X_train = X_subset[Not(i), :]
		y_train = data[subset, "Occupancy"][Not(i)]
		lda = LinearDiscriminantAnalysis()
		transformed_lda = lda.fit(X_train, y_train)
		ŷ = lda.predict(X_test)[1]
		if ŷ==1 && data[:, "Occupancy"][id]==1
			occupied_acc += 1
		elseif ŷ==0 && data[:, "Occupancy"][id]==0
			unoccupied_acc += 1
		end
	end		
end

# ╔═╡ d7dffc13-3be1-4eb1-aec2-8884f9eb46bd
begin
	n_occupied = sum(data[subset, "Occupancy"].==1)
	n_unoccupied = sum(data[subset, "Occupancy"].==0)
end

# ╔═╡ fc034ade-d1fe-454b-8e78-f491bd0f5847
jackknife = DataFrame(:State =>["Occupied", "Unoccupied"], 
		  :Occupied =>[occupied_acc, n_unoccupied - unoccupied_acc], 
		  :Unoccupied => [n_occupied - occupied_acc, unoccupied_acc], 
		  :Accuracy => [occupied_acc / n_occupied, unoccupied_acc /n_unoccupied])
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "b6b229567c5a5b31e59f2525f87704b5d7e9a900"
...
git-tree-sha1 = "b32c4b415f41f10c671cba02ae3275027dea8892"
